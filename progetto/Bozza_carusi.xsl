      ...
      <body>
        <div class="header-info">
          ...
          <p>
            ...
            <xsl:if test="$publisher_facs">
              <xsl:attribute name="data-zone-id">
                <xsl:value-of select="substring-after($publisher_facs, '#')"/>
              </xsl:attribute>
              <xsl:attribute name="class">highlightable-metadata</xsl:attribute>
            </xsl:if>
            <strong>Casa editrice: </strong>
            <xsl:value-of select="$publisher_node"/>
          </p>
